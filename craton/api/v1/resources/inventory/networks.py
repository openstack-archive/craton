...
class Networks(base.Resource):
    ...
    def get(self):
        ...
        try:
            ...
            obj = dbapi.networks_get_by_region(context, region, filters)
...
class NetDevices(base.Resource):
    ...
    def get(self):
        ...
        # This is a query constraint, you cant fetch all net devices
        # for all regions, you have to query devices by region.
        if not region:
        ...
        try:
            LOG.info("Getting all network devices that match filters %s"
                     % filters)
            obj = dbapi.netdevices_get_by_region(context, region, filters)
        except exceptions.NotFound:
            return self.error_response(404, 'Not Found')
        except Exception as err:
...
class NetInterfaces(base.Resource):
    ...
    def get(self):
        ...
        if not host:
            return self.error_response(400, "Missing `host` in query")

        try:
            LOG.info("Getting all network interface that match filters %s"
                     % filters)
            obj = dbapi.net_interfaces_get_by_host(context, host, filters)
        except exceptions.NotFound:
            return self.error_response(404, 'Not Found')
        except Exception as err:
