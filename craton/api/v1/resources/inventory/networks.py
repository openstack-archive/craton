...
class NetDevices(base.Resource):
    ...
    def get(self):
        ...
        try:
            LOG.info("Getting all netwoork devices that match filters %s"
                     % filters)
...
class NetInterfaces(base.Resource):
    ...
    def get(self):
        ...
        try:
            LOG.info("Getting all netwoork interface that match filters %s"
                     % filters)
            obj = dbapi.net_interfaces_get_by_region(context, region, filters)
        except exceptions.NotFound:
            return self.error_response(404, 'Not Found')
        except Exception as err:
            LOG.error("Error during net devices get: %s" % err)
