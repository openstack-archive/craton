...
class Users(base.Resource):
    ...
    def get(self):
        """Get all the user. This is project admin only function."""
        user_id = g.args["id"]
    ...
    def post(self):
        """Create a new user. This is project admin only function."""
        context = request.environ.get('context')
        try:
...
class UserById(base.Resource):
    ...
    def get(self, id):
        ...
        context = request.environ.get('context')
        try:
            user_obj = dbapi.proejcts_get_by_id(context, id)
        except exceptions.NotFound:
            return self.error_response(404, 'Not Found')
        except Exception as err:
...
class UserData(base.Resource):

    def put(self, id):
        """
        Update existing user data, or create if it does
        not exist.
        """
        context = request.environ.get('context')
        try:
            dbapi.users_data_update(context, id, request.json)
        except Exception as err:
            LOG.error("Error during user data update: %s" % err)
            return self.error_response(500, 'Unknown Error')
    ...
    def delete(self, id):
        ...
        context = request.environ.get('context')
        try:
            dbapi.users_data_delete(context, id, request.json)
        except Exception as err:
            LOG.error("Error during users data delete: %s" % err)
            return self.error_response(500, 'Unknown Error')
